    ...
    jQuery('form.contact_1', '.sc_form_style_form_1').on('submit', function(e) {
        ...
        for (var i = formFields.length - 1; i >= 0; i--) {
            if (!formFields[i].value.length) {
                ...
                $form.find('[name="' + formFields[i].name + '"]', '.sc_form_style_form_1').addClass('trx_addons_field_error').on('focus', function() {
                    jQuery(this).removeClass('trx_addons_field_error')
                });
            };
            if (!emailReg.test(emailaddressVal)) {
                $form.find('#contact_form_email', '.sc_form_style_form_1').addClass('trx_addons_field_error').on('focus', function() {
                    jQuery(this).removeClass('trx_addons_field_error')
                });
            }
        };
        ...
        if ($form.find('#contact_form_email', '.sc_form_style_form_1').hasClass('trx_addons_field_error')) {
            if ($form.find('div.sc_form_result', '.sc_form_style_form_1').hasClass('error_email_mask')) {} else {
